...
from .rank_zero import rank_zero_only, rank_zero_warn, rank_prefixed_message
...
def seed_everything(seed: Optional[int] = None, workers: bool = False) -> int:
    ...
    """
    if seed is None:
        env_seed = os.environ.get("HYDRA_GLOBAL_SEED")
        if env_seed is None:
            seed = 0
    ...
    os.environ["HYDRA_GLOBAL_SEED"] = str(seed)
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)

    os.environ["HYDRA_SEED_WORKERS"] = f"{int(workers)}"
